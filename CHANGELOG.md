...
- **`buildRegistrationMessage` now encodes `InstKind::RegisterBlsKey` as variant `4`, not `3`** — Kontor added an `UpdateProvenance` variant between `Call` and `Issuance` (2026-06-25), shifting every variant after it by one. Horizon-Portal picked the change up when it repinned to Kontor `b8e40260`, so the registration signature this client produced no longer verified: the Portal rebuilt the message with `0x04` at that offset and rejected the signature with `REGISTRATION_SIGNATURE_VERIFICATION_FAILED`. `InstKind::Call` is unaffected — it is still variant `1`, so uploads and NFT mints were never broken.

### Compatibility

- **0.2.11 requires a Horizon-Portal deployment built against Kontor `b8e40260` or later** (any revision that includes `InstKind::UpdateProvenance`). The registration message is signed over a variant index, so the client and the Portal must agree on `InstKind`'s declaration order — there is no negotiation and no fallback. Against an older Portal, 0.2.11 fails registration with `REGISTRATION_SIGNATURE_VERIFICATION_FAILED`, exactly as 0.2.10 does against a current one. Pin `0.2.10` if you talk to a Portal that has not repinned yet. Login on an already-registered signer, uploads, and NFT mints are unaffected either way — they go through `InstKind::Call`, whose variant index did not move.

### Added

- **`Agreement.ledger_index`** — the append-only slot the filestorage contract assigns a file in Kontor's aggregated ledger, returned by `GET /api/agreements` and `GET /api/agreements/{id}`. Since kontor-crypto 0.3.0 it is a public input of the Proof-of-Retrievability SNARK, so a storage node must place the file at exactly this index. `null` until the creating Bitcoin transaction is confirmed and reconciled against Kontor; `undefined` on Portal deployments predating the field.
- **Cross-language postcard vectors** in `postcard.test.ts` — two byte-exact fixtures generated by Kontor's own `Inst::aggregate_signing_message` (at the revision Horizon-Portal pins, `b8e40260`), one per op kind. They pin this client's encoder: any edit to `postcard.ts` that changes the bytes — a variant index, the tuple layout, a length prefix — fails here instead of at a user's registration. They do **not** detect drift on Kontor's side. The expected bytes are static, so a future reordering of `SignerRef` or `InstKind` upstream leaves these tests green while registration breaks in production, exactly as it did this time. Catching that needs the vectors regenerated against the revision Horizon-Portal currently resolves — note that the Portal tracks `branch = "main"`, not a fixed rev, so that revision moves on any `cargo update`.

### Notes

- **No change needed for `kontor-crypto` 0.3.0.** Horizon-Portal now builds against the Rust crate `kontor-crypto` 0.3.0, but that crate is the prover/verifier and depends on `kontor-crypto-core` **0.2.0** — the crate that owns `prepare_file`, the Merkle tree, and the Reed–Solomon parameters, and the one `@kontor/kontor-crypto` compiles to WASM. The published `kontor-crypto-core` 0.2.0 sources are byte-identical to the Kontor-Crypto checkout the npm package is built from, and `prepare_file` under 0.3.0 produces the same `root` / `file_id` / `object_id` / `padded_len` as the browser module for every size class tested. The `^0.2.0` peer range stays correct; see "Which crypto version this tracks" in the README.
