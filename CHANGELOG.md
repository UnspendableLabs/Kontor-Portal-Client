...
- **Cross-language postcard vectors** in `postcard.test.ts` — two byte-exact fixtures generated by Kontor's own `Inst::aggregate_signing_message` (at the revision Horizon-Portal pins), one per op kind. A future reordering of `SignerRef` or `InstKind` now fails a test here instead of a user's login.
